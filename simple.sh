...
if [ ! -f ~/typer/wc.t ]; then
	ssh lumet@ssh.lumet.nichost.ru 'cat /home/lumet/typer/wc.m > ./config/local_wc.m'
	#sudo echo "30" > ~/typer/wc.t
	echo "CREATED WORDS COUNT FILE"
	sleep 4
fi
if [ ! -d /media/$USER/RASPBERRYPI ]; then
	echo -e "NO USB DRIVE MOUNTED! MOUNT DRIVE AND RELAUNCH SCRIPT!"
	sleep 4
	exit
fi
############################
#wc=$(cat ./config/local_wc.m)
...
vfolder="/media/$USER/RASPBERRYPI/Videos/"
options=($(ls $vfolder))
